...
function showWindowHref() {
    let url_useful = url.value.split("&");
    url_useful[0] = url_useful[0].substr(url_useful[0].indexOf("?") + 1, url_useful[0].length - url_useful[0].indexOf("?"));
    let boolean = false;
    let url_output;
    for (let i = 0; i < url_useful.length; i++) {
        let str_ele = url_useful[i].split("=");
        if (str_ele[0] === "name") {
            boolean = true;
...
function arrSameStr() {
    ...
    for (let i = 0; i < most.value.length; i++) {
        ...
        if (num > max_num) {
            max_num = num;
            max_char = most.value[i];
        }
    }
    result.value = "The most character is:" + max_char + " times:" + max_num;
}